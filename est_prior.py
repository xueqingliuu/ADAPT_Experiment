...
For every model below we:

  1. Fit a per-user ridge regression on that user's rows of ``df_fit`` to obtain
     ``theta_u`` (per-user posterior mean) and ``sigma2_u`` (residual variance).
  2. Pool across users:

         prior mean    = mean_u  theta_u
         prior cov     = diag( var_u(theta_u) )       (across-user variance,
                                                       one number per coefficient)
         sigma2        = mean_u sigma2_u
...
``experiment.py`` ``get_pf_data``):
  ...
  2. ``fourSC_lag1`` is cleared at the first slot of every week (Monday AM)
     and ``anticipated_affect_yesterday`` is cleared at the first day
     (Monday), matching the PF's week-boundary lag clearing.
  3. ``CAE_avg_lastweek`` (in fourSC/antic/CAE) and ``perceived_utility_lastweek``
...
RL bottleneck V_alpha           -> mu_0_bottleneck, Sigma_0_bottleneck,
   (build_phi_bottleneck)          sigma2_bottleneck
RL reward shaping eta           -> mu_0_reward, Sigma_0_reward, sigma2_reward
   (build_phi_action_rewardshaping)
RL Q (no TD modify)             -> mu_0_micro, Sigma_0_micro, sigma2_rl_micro
   (build_phi_action, fitted Q
    bootstrapping next-week Q)
RL Q (with TD modify)           -> mu_0_micro_mtd, Sigma_0_micro_mtd,
...
with ``R_{k+1} = CAE_avg_norm`` of week k (the env's reward_fn) and
...
def _week_fix_and_filter(df: pd.DataFrame) -> pd.DataFrame:
    """Match ``4_perceived_utility`` week relabel and ``week < 13`` filter."""
    df = df.copy()
...
def _pool_user_fits(
    ...
    """Aggregate per-user posterior means into (prior mean, diag cov, sigma2)."""
    ...
    if arr.shape[0] > 1:
        ...
        var = np.ones(arr.shape[1])
    var = np.maximum(var, MIN_SIGMA2)
    cov = np.diag(var)
    s2 = [s for s in user_sigma2s if s is not None]
    sigma2 = float(np.mean(s2)) if s2 else 1.0
    return mean, cov, sigma2
...
# ──────────────────────────────────────────────────────────────────
THETA_FOURSC_NAMES = [
    "intercept", "fourSC_lag1", "yesterday_step", "seven_day_step_count_avg",
    "prior2hour_step", "Previous7DaysRPA", "recent_burden",
    "seven_day_pageview", "past7days_morning_wearing",
    "yesterday_salience_message", "Interacted_7d_walk",
    "Interacted_7d_salience", "anticipated_affect_yesterday",
    "dow", "decision_time", "perceived_utility_lastweek", "CAE_avg_lastweek",
    "Ah", "Ah*yesterday_step", "Ah*prior2hour_step", "Ah*recent_burden",
    "Ah*seven_day_pageview", "Ah*past7days_morning_wearing",
    "Ah*yesterday_salience_message", "Ah*Interacted_7d_walk",
    "Ah*Interacted_7d_salience", "Ah*anticipated_affect_yesterday",
    "Ah*dow", "Ah*decision_time",
    "Ah*perceived_utility_lastweek", "Ah*CAE_avg_lastweek",
]

THETA_ANTIC_NAMES = [
    "intercept", "anticipated_affect_yesterday", "today_step",
    "recorded_physical_activity", "salience_message", "dow",
    "perceived_utility_lastweek", "CAE_avg_lastweek",
    "ws_morning", "ws_afternoon",
    "ws_morning*today_step", "ws_afternoon*today_step",
    "ws_morning*RPA", "ws_afternoon*RPA",
    "ws_morning*salience_message", "ws_afternoon*salience_message",
    "ws_morning*dow", "ws_afternoon*dow",
    "ws_morning*perceived_utility_lastweek",
    "ws_afternoon*perceived_utility_lastweek",
    "ws_morning*CAE_avg_lastweek", "ws_afternoon*CAE_avg_lastweek",
]

THETA_CAE_NAMES = (
    ["intercept", "CAE_avg_lastweek", "week_norm"]
    + [f"fourSC_slot_{j}" for j in range(14)]
    + [f"antic_day_{j}" for j in range(7)]
)

THETA_CAE_SHORT_NAMES = ["intercept", "CAE_avg"]


# PF-specific column indices (must match experiment.py):
#   * ``foursc_lag1_col = 1``: AR-1 lag in fourSC, cleared at Monday AM (slot 0).
#   * ``antic_lag1_col  = 1``: anticipated_affect_yesterday in antic, cleared
#     at Monday (day 0).
_PF_FOURSC_LAG1_COL = 1
...
def _build_fourSC_design(dat: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Differs from ``gen_fourSC_mean`` in two PF-specific ways:
      ...
      * ``fourSC_lag1`` is set to 0 at the first slot of each week
        (Monday AM), matching the PF's week-boundary lag clearing.
    ...
    ``CAE_avg_lastweek_norm`` column.
    """
    int_ = np.ones(len(dat))
    fSC_lag1 = _fill_nan(dat["FourSC_lag1"].to_numpy())
    ...
    rb = _fill_nan(dat["recent_burden_norm"].to_numpy())
    seven_pv = _fill_nan(dat["Past7DaysPageviewEMA_norm"].to_numpy())
    ...
    i7s = _fill_nan(dat["Interacted_7d_salience"].to_numpy())
    aay = _fill_nan(dat["anticipated_affect_yesterday_norm"].to_numpy())
    dow = dat["dow_norm"].to_numpy()
    ...
    X = np.column_stack([
        int_, fSC_lag1, yest_step, seven_step, prior2, prev7rpa, rb,
        seven_pv, past7_wear, y_sal, i7w, i7s, aay, dow, dt_, pu, cae,
        Ah, Ah * yest_step, Ah * prior2, Ah * rb, Ah * seven_pv,
        Ah * past7_wear, Ah * y_sal, Ah * i7w, Ah * i7s, Ah * aay,
        Ah * dow, Ah * dt_, Ah * pu, Ah * cae,
    ])
    ...
    # Clear AR-1 lag at the first slot of every week (Monday AM).
    X[:, 0, _PF_FOURSC_LAG1_COL] = 0.0
...
def _build_antic_design(dat: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    the regression population parameter, so actual ``CAE_avg_lastweek_norm``
    ...
    X = np.column_stack([
        int_, antic_yest, today_step, rpa, sal_msg, dow, pu, cae,
        ws_m, ws_a,
        ws_m * today_step, ws_a * today_step,
        ws_m * rpa, ws_a * rpa,
...
def fit_pf_priors(df_fit: pd.DataFrame) -> Dict[str, Any]:
    """Per-user PF / Y / tY regression -> pooled priors."""
    ...
    for uid, dat in by_user:
        ...
        fSC_thetas.append(th); fSC_s2.append(s2)

        Xa, ya = _build_antic_design(dat)
        th, s2 = _ridge_fit(Xa, ya, alpha=RIDGE_ALPHA_PF)
        ant_thetas.append(th); ant_s2.append(s2)

        XC, yC = _build_CAE_design(dat)
        th, s2 = _ridge_fit(XC, yC, alpha=RIDGE_ALPHA_PF)
        Y_thetas.append(th); Y_s2.append(s2)

        Xs, ys = _build_CAE_short_design(dat)
        th, s2 = _ridge_fit(Xs, ys, alpha=RIDGE_ALPHA_PF)
        tY_thetas.append(th); tY_s2.append(s2)

    nu_fSC, G_fSC, s2_fSC = _pool_user_fits(fSC_thetas, fSC_s2)
    nu_ant, G_ant, s2_ant = _pool_user_fits(ant_thetas, ant_s2)
    nu_Y,   G_Y,   s2_Y   = _pool_user_fits(Y_thetas,  Y_s2)
    nu_tY,  G_tY,  s2_tY  = _pool_user_fits(tY_thetas, tY_s2)
...
def _build_rl_context_vector_from_row(row) -> np.ndarray:
    ...
    return np.array([
        ...
        float(row["recent_burden_norm"]),
        float(row["Past7DaysPageviewEMA_norm"]),
        float(row["yesterday_SalienceMessage"]),
        float(row["Interacted_7d_walk"]),
        float(row["Interacted_7d_salience"]),
        float(row["anticipated_affect_yesterday_norm"]),
    ], dtype=float)
...
def _user_weekly_tensors(dat: pd.DataFrame) -> Dict[str, np.ndarray]:
    ...
    is Monday .. Sunday (dow = 1 .. 7).  Rows are NaN-filled with the
    ...
    cols = [
        ...
        "Previous7DaysRPA", "recent_burden_norm", "Past7DaysPageviewEMA_norm",
        "yesterday_SalienceMessage", "Interacted_7d_walk",
        "Interacted_7d_salience", "anticipated_affect_yesterday_norm",
    ...
    p_C = 10                 # build_rl_context_vector dim
    ...
    for k in range(n_w):
        w0 = k * K_SLOTS_WEEK
        # First row of week: gives perceived_utility_lastweek + CAE_avg_lastweek_norm
        ...
        # Per-week mediator memory: fourSC (d, AM/PM), antic (d), pageview, fitbit, daily_present.
        for d in range(1, 7):
...
def fit_bottleneck_prior(
    ...
    """Per-user regression of weekly CAE on phi_bottleneck(S_{k,0}).

    Returns the pooled prior plus the per-user alpha vectors (used as the
    V_alpha target when fitting the TD-modify Q prior).
    """
...
def fit_reward_prior(df_fit: pd.DataFrame) -> Dict[str, Any]:
    """Per-user regression of weekly CAE on the sum of phi_rs across slots.

    Replicates ``build_reward_shaping_training_data``: one row per week with
    feature = sum_{d,t} phi_rs(S_{k,d,t}).
    """
    thetas: List[Optional[np.ndarray]] = []
    sigma2s: List[Optional[float]] = []
    for _uid, dat in df_fit.groupby("ParticipantIdentifier", sort=False):
        ...
        rows = []
        for k in range(n_w):
            phi_week = np.zeros_like(_phi_rs(td, k, 0))
            for rl_idx in range(N_RL_SLOTS_WEEK):
                phi_week = phi_week + _phi_rs(td, k, rl_idx)
...
# ──────────────────────────────────────────────────────────────────
def _fitted_q_one_user(
    td: Dict[str, np.ndarray],
    alpha_user: Optional[np.ndarray],
    use_td_modify: bool,
    n_iters: int = N_FQI_ITERS,
) -> Tuple[Optional[np.ndarray], Optional[float]]:
    """FQI for one participant; returns (theta_Q, sigma2_Q) or (None, None).

    Drops the last week (no next-week bootstrap state available). Terminal
    bootstrap is from phi_bottleneck @ alpha_user when ``use_td_modify``
    is True; otherwise from next-week (1, 1) Q.
    """
    n_w = td["n_w"]
    if n_w < 2:
        return None, None
    ...
    phi_next_bottleneck = np.zeros((n_train, alpha_user.size)) if (
        use_td_modify and alpha_user is not None
    ) else None
    for k in range(n_train):
        phi_next_a0[k] = _phi_action(td, k + 1, 0, 0)
        phi_next_a1[k] = _phi_action(td, k + 1, 0, 1)
        if phi_next_bottleneck is not None:
            phi_next_bottleneck[k] = _phi_bottleneck_start(td, k + 1)

    R = td["R_week"][:n_train]

    n_rows = n_train * N_RL_SLOTS_WEEK
    X = phi_obs.reshape(n_rows, p_phi)
        ...
        for k in range(n_train):
            for idx in range(N_RL_SLOTS_WEEK):
                if idx < N_RL_SLOTS_WEEK - 1:
                    ...
                    q0 = phi_a0[k, idx + 1] @ theta
                    q1 = phi_a1[k, idx + 1] @ theta
                    targets[k, idx] = GAMMA_DT_SCALAR * max(q0, q1)
                else:
                    # terminal (6, 2): reward + γ_terminal · bootstrap
                    if use_td_modify and alpha_user is not None:
                        boot = float(phi_next_bottleneck[k] @ alpha_user)
                    else:
                        q0 = phi_next_a0[k] @ theta
                        q1 = phi_next_a1[k] @ theta
                        boot = float(max(q0, q1))
                    targets[k, idx] = R[k] + GAMMA_DT_SCALAR * boot
        y = targets.reshape(n_rows)
        theta_new, _ = _ridge_fit(X, y, alpha=RIDGE_ALPHA_RL)
    ...
    resid = y - X @ theta
    sigma2 = max(float(np.var(resid, ddof=1)), MIN_SIGMA2) if resid.size > 1 else 1.0
    return theta, sigma2
...
def fit_q_priors(
    ...
    """Fit both Q-function priors (without and with TD-modify)."""
    th_nomod, s2_nomod = [], []
    th_mod,   s2_mod   = [], []
    ...
    for uid, dat in df_fit.groupby("ParticipantIdentifier", sort=False):
        ...
        th, s2 = _fitted_q_one_user(td, alpha_user=None, use_td_modify=False)
        th_nomod.append(th); s2_nomod.append(s2)

        th, s2 = _fitted_q_one_user(td, alpha_user=alpha_u, use_td_modify=True)
        th_mod.append(th);   s2_mod.append(s2)

    mu_n, Sigma_n, sigma2_n = _pool_user_fits(th_nomod, s2_nomod)
    mu_m, Sigma_m, sigma2_m = _pool_user_fits(th_mod,   s2_mod)
    return (
...
def main() -> Dict[str, Any]:
    ...
    q_no_mod, q_mod = fit_q_priors(df_fit, alpha_by_user)
