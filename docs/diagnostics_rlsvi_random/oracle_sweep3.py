    ...
    for k in range(nweek):
        ...
        for d in range(ex.N_RL_DAYS):
            for t in range(ex.N_RL_SLOTS):
                ...
                if policy.startswith("bern"):
                ...
                elif policy in ("timing","cov","v1map","v10map","v1map_t","ptiming","slotonly","dayonly"):
                    s_ = oenv.s
                    ...
                    elif policy == "ptiming": score = FIT["ptiming"][d, t]
                    elif policy == "slotonly": score = FIT["timing"][uid][:, t].mean()
                    elif policy == "dayonly": score = FIT["timing"][uid][d, :].mean()
...
POL = ["bern0.5","ptiming","slotonly","dayonly"]
...
PT = np.mean([FIT["timing"][u] for u in UIDS_ALL], axis=0); FIT["ptiming"] = PT
FIT["tau_ptiming"] = {u: float(np.median(PT)) for u in UIDS_ALL}
FIT["tau_slotonly"] = {u: float(FIT["timing"][u].mean()) for u in UIDS_ALL}   # send in the better slot type only -> rate 0.5
FIT["tau_dayonly"] = {u: float(np.median(FIT["timing"][u].mean(axis=1))) for u in UIDS_ALL}
print("pooled timing table (rows Mon..Sat, cols AM PM), normalized CAE units:"); print(np.round(PT*1e3,3))
print("per-user AM-minus-PM mean Δ (x1e3):", np.round([ (FIT["timing"][u][:,0]-FIT["timing"][u][:,1]).mean()*1e3 for u in UIDS_ALL],2))
...
for i, uid in enumerate(UIDS):
    ...
    print(f"uid {uid} done {time.time()-t0:.0f}s  tau={tau_u[uid]:.2e}  ptiming-rate={rate['ptiming'][i].mean():.3f}", flush=True)
