...
POL = ["bern0.0","bern0.25","bern0.5","bern0.75","bern1.0","sign","rank","antirank"]
...
for i, uid in enumerate(UIDS):
    ...
    tau_u[uid] = float(np.median(rows[:, 5]))
    ...
    print(f"uid {uid} done {time.time()-t0:.0f}s  tau={tau_u[uid]:.2e}  rank-rate={rate['rank'][i].mean():.3f}", flush=True)
